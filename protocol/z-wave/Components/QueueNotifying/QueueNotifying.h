...
typedef struct SQueueNotifying
{
  ...
  TaskHandle_t ReceiverTask;            /**< Handle to queue receiver task where
...
*/
void QueueNotifyingInit(SQueueNotifying* pThis, QueueHandle_t Queue, TaskHandle_t ReceiverTask, uint8_t iTaskNotificationBitNumber);
...
*/
EQueueNotifyingStatus QueueNotifyingSendToBackFromISR(SQueueNotifying* pThis, const uint8_t* pItem, BaseType_t *pxHigherPriorityTaskWoken);
