...
/**
* Typedef needed to include pointer for struct SSwTimer in struct
...
typedef struct  SSwTimerLiaison
{
  ...
  SSwTimer**	pTimerList;             /**< Pointer to array of pointers to SwTimer objects */
  StaticEventGroup_t ReceiverEvent;   /**< FreeRTOS event group for keeping
                                           track of pending timer callbacks */
  uint8_t iTaskNotificationBitNumber; /**< which bit to use when notifying receiver
                                           task of pending timer event(range 0 - 31) */
  TaskHandle_t ReceiverTask;          /**< Handle to task in which callbacks should be performed */
} SSwTimerLiaison;
...
*/
void TimerLiaisonSetReceiverTask(SSwTimerLiaison* pThis, TaskHandle_t ReceiverTask);
...
*
* @param[in]     TimerHandle  FreeRTOS timer handle for the timer that timed out
*/
void TimerLiaisonExpiredTimerCallback(TimerHandle_t TimerHandle);
