...
typedef struct _LEARN_INFO_
{
  ...
  uint8_t  bSource;      /* Node id of the node that send node info */
...
{
  uint8_t      homeID[4];              /* HomeID the Nodeinfo was received with */
...
typedef struct _CONTROLLER_UPDATE_INCLUDED_NODE_INFORMATION_FRAME_
{
  ...
  uint8_t                  abINIFsmartStartNWIHomeID[4]; /* homeID used when using Smart Start */
