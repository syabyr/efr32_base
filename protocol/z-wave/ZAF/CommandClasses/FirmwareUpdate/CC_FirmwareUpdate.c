  ...
  uint8_t cmdLength)
{
  ...
  switch (pCmd->ZW_Common.cmd)
  {
    ...
    case FIRMWARE_UPDATE_MD_REPORT_V5:
      {
        uint16_t crc16Result = CRC_CheckCrc16(0x1D0F, &(pCmd->ZW_Common.cmdClass), cmdLength);
        ...
        uint8_t fw_actualFrameSize =  cmdLength -
                                  ...
                                  (sizeof(pCmd->ZW_FirmwareUpdateMdReport1byteV5Frame.cmdClass) +
  ...
  VOID_CALLBACKFUNC(pCbFunc)(TRANSMISSION_RESULT * pTransmissionResult))
{
  ...
  pTxBuf->ZW_FirmwareUpdateMdStatusReportV5Frame.waittime1 = (waitTime >> 8);
  pTxBuf->ZW_FirmwareUpdateMdStatusReportV5Frame.waittime2 = (waitTime & 0xff);
  ...
  uint16_t firmwareUpdateReportNumber)
{
  ...
  DPRINTF("CmdClassFirmwareUpdateMdGet %d ", firmwareUpdateReportNumber);
  ...
  pTxBuf->ZW_FirmwareUpdateMdGetV5Frame.numberOfReports = 1;
  pTxBuf->ZW_FirmwareUpdateMdGetV5Frame.properties1 = firmwareUpdateReportNumber >> 8;
  pTxBuf->ZW_FirmwareUpdateMdGetV5Frame.reportNumber2 = firmwareUpdateReportNumber & 0xFF;
  if(EQUEUENOTIFYING_STATUS_SUCCESS != Transport_SendRequestEP(
      (uint8_t *)pTxBuf,
...
REGISTER_CC(COMMAND_CLASS_FIRMWARE_UPDATE_MD_V5, FIRMWARE_UPDATE_MD_VERSION_V5, handleCommandClassFWUpdate);
