...
/* UART1 GPIO is defined in extension_board_xxx.h */
 ...
 *
 * @param pin  GPIO pin causing the interrupt.
 */
static void ButtonPressCallback(uint8_t pin)
{
...
void Board_EnableButton(button_id_t btn)
{
  ...
  if (Board_IsButtonAvailable(btn))
  {
    ...
    GPIOINT_CallbackRegister(pin, ButtonPressCallback);
  }
}
...
#if defined(VCOM_ENABLE) && defined(VCOM_ENABLE_PORT)
