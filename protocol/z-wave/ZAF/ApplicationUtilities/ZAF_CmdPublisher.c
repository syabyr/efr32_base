...
void ZAF_CP_CommandPublish(CP_Handle_t handle, SZwaveReceivePackage* pRxPackage)
{
...
//            pContext->subscriberList[i].CmdClass, pContext->subscriberList[i].Cmd);
    switch (pContext->subscriberList[i].type)
    {
      case CP_SUBSCRIBER_TYPE_ALL:
        pContext->subscriberList[i].pFunction(pContext->subscriberList[i].pSubscriberContext,pRxPackage);
        break;
      case CP_SUBSCRIBER_TYPE_CC:
        if (pRxPackage->uReceiveParams.Rx.Payload.rxBuffer.ZW_Common.cmdClass == pContext->subscriberList[i].CmdClass)
        {
          pContext->subscriberList[i].pFunction(pContext->subscriberList[i].pSubscriberContext,pRxPackage);
        }
        break;
      case CP_SUBSCRIBER_TYPE_CMD:
        if (pRxPackage->uReceiveParams.Rx.Payload.rxBuffer.ZW_Common.cmdClass == pContext->subscriberList[i].CmdClass
            && pRxPackage->uReceiveParams.Rx.Payload.rxBuffer.ZW_Common.cmd == pContext->subscriberList[i].Cmd)
        {
          pContext->subscriberList[i].pFunction(pContext->subscriberList[i].pSubscriberContext,pRxPackage);
        }
        break;
