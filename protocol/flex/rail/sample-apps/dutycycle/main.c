...
void slaveTimerExpiredCallback(sl_sleeptimer_timer_handle_t *handle, void *data);
...
sl_sleeptimer_timer_handle_t slaveRtcId;  // RTC timer ID
...
static void switchSlaveAppState(void)
{
  ...
  switch (appState) {
    case IDLE:
      sl_sleeptimer_restart_timer(&slaveRtcId, SLAVE_OFF_TIME, slaveTimerExpiredCallback, NULL, 0u, 0u);
      RAIL_Idle(railHandle, RAIL_IDLE, true);
      EMU_EnterEM2(true);
      break;
    case PACKET_RX:
      RAIL_Idle(railHandle, RAIL_IDLE, true);
      RAIL_StartRx(railHandle, channel, NULL);
      sl_sleeptimer_restart_timer(&slaveRtcId, SLAVE_ON_TIME, slaveTimerExpiredCallback, NULL, 0u, 0u);
      break;
    case PACKET_TX:
    ...
    case SLAVE_BLAST_RX:
      RAIL_Idle(railHandle, RAIL_IDLE, true);
      RAIL_StartRx(railHandle, channel, NULL);
      sl_sleeptimer_restart_timer(&slaveRtcId, SLAVE_BLAST_RX_DELAY, slaveTimerExpiredCallback, NULL, 0u, 0u);
      break;
    default:
    {
      lcdPrintError(INVALID_APP_STATE);
...
void slaveTimerExpiredCallback(sl_sleeptimer_timer_handle_t *handle, void *data)
{
  switch (appState) {
    ...
    case SLAVE_BLAST_RX: // blast RX finished, so wait a bit and then ACK
      sl_sleeptimer_delay_millisecond(2 * SLAVE_BLAST_RX_DELAY);
      appState = PACKET_TX;
      break;
    default:
    {
...
void RAILCb_Generic(RAIL_Handle_t railHandle, RAIL_Events_t events)
{
  ...
  if (events & (RAIL_EVENT_RX_PREAMBLE_DETECT
    ...
    if (appMode == DUTY_CYCLE_MODE) {
      ...
      sl_sleeptimer_stop_timer(&slaveRtcId);
      appState = SLAVE_BLAST_RX;
      forceStateChange = true;
    }
  }
