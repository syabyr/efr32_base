 ...
 * -# The application calls ::emberCommandReaderInit() to initialize and
 ...
 * is correct.  It performs any conversions necessary, for example,
...
/** Indicate whether or not the command entry structure includes descriptions
...
/** Retrieve quoted string or hexadecimal string arguments.
...
/** Copy the string argument to the given destination up to maxLength.
 ...
 * such as EUI64 or key data and left padding them with zeros.
...
/** Parse and return, via target, an IP address at the provided index.
 * Returns true if an IP address was successfully parsed
 * Returns false otherwise.
 */
bool emberGetIpArgument(uint8_t index, uint8_t *target);

/** Parse and return, via target, an IP prefix at the provided index.
...
/** Copy the EUI64 argument to the given EmberEui64 destination reversing the
...
/** Copy the extended PAN ID argument to the given destination, reversing the
...
/** @brief Process the input coming in on the given serial port.
...
/** @brief Return the value of a hexadecimal digit ch (0 - 15).
 * Returns a value > 15 if ch is not a hexadecimal digit.
 */
uint8_t emberHexToInt(uint8_t ch);

/** @brief Parse textual representations of IPv6 addresses as described in
...
/** @brief Parse textual representations of IPv6 prefixes as described in
 * http://www.ietf.org/rfc/rfc4291.txt.
 */
