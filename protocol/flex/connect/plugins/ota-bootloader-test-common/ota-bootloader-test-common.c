...
bool otaResumeStartCounterReset = DEFAULT_COUNTER_RESET;
//------------------------------------------------------------------------------
// CLI commands.
...
void bootloaderFlashEraseCommand(void)
{
  ...
  otaResumeStartCounterReset = true;
}
...
{
  uint8_t newImageTag = 0;
  newImageTag = emberUnsignedCommandArgument(0);
  if (newImageTag != otaBootloaderTestImageTag) {
    otaBootloaderTestImageTag = newImageTag;
    otaResumeStartCounterReset = true;
  }
