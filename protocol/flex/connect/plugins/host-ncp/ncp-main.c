...
void ncpMain(void* pvParameters)
{
  ...
  while (true) {
    ...
    ncpLinkProcessIncoming();  // Process incoming messages from the HOST
    emberTick();
...
int MAIN(void)
{
  halInit();
  INTERRUPTS_ON();
  emberSerialInit((uint8_t)APP_SERIAL,
                  (SerialBaudRate)APP_BAUD_RATE,
                  (SerialParity)PARITY_NONE, 1);
  ...
  // Call the NCP main function. This should never return
  ncpMain(NULL);
