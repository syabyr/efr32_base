...
static void sendCommissioningReply(EmberGpAddress *gpdAddr,
                                   ...
                                   GpCommDataSaved *commissioningGpd)
{
  ...
  if (commissioningState.unicastCommunication) { //  based on the commission mode as decided by sink
    retval = emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT, gppShortAddress);
  } else {
    retval = emberAfSendCommandBroadcast(EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS);
  }
                                      ...
                                      GpCommDataSaved *commissioningGpd)
{
  ...
  // -------------------------------------------------
  bool gpdFixed = (gpdfOptions & EMBER_AF_GP_GPD_COMMISSIONING_OPTIONS_FIXED_LOCATION) \
                  ? true : false;
  bool gpdMacCapabilities = (gpdfOptions & EMBER_AF_GP_GPD_COMMISSIONING_OPTIONS_MAC_SEQ_NUM_CAP) \
                            ? true : false;
  ...
  if (sinkEntryIndex != 0xFF) {
    ...
    pairingOptions = EMBER_AF_GP_PAIRING_OPTION_ADD_SINK
                     ...
                     | (gpdFixed << EMBER_AF_GP_PAIRING_OPTION_GPD_FIXED_OFFSET)
                     | (gpdMacCapabilities << EMBER_AF_GP_PAIRING_OPTION_GPD_MAC_SEQUENCE_NUMBER_CAPABILITIES_OFFSET)
                     | (commissioningGpd->securityLevel << EMBER_AF_GP_PAIRING_OPTION_SECURITY_LEVEL_OFFSET)
                     | (commissioningGpd->securityKeyType << EMBER_AF_GP_PAIRING_OPTION_SECURITY_KEY_TYPE_OFFSET)
                     | ((gpdMacCapabilities || commissioningGpd->securityLevel) ? EMBER_AF_GP_PAIRING_OPTION_GPD_SECURITY_FRAME_COUNTER_PRESENT : 0)
                     | ((commissioningGpd->securityLevel && commissioningGpd->securityKeyType) ? EMBER_AF_GP_PAIRING_OPTION_GPD_SECURITY_KEY_PRESENT : 0)
                     | (sinkTableAssignedAliasNeeded << EMBER_AF_GP_PAIRING_OPTION_ASSIGNED_ALIAS_PRESENT_OFFSET)
                     | ((sinkCommunicationMode == EMBER_GP_SINK_TYPE_D_GROUPCAST
                         || sinkCommunicationMode == EMBER_GP_SINK_TYPE_GROUPCAST)
                        ? EMBER_AF_GP_PAIRING_OPTION_GROUPCAST_RADIUS_PRESENT : 0);
    ...
    if (commissioningGpd->doNotSendGpPairing) {
      ...
      indexOrDestination = emberAfGetNodeId();
      #endif
    } else {
      if (commissioningState.unicastCommunication) { //  based on the commission mode as decided by sink
        type = EMBER_OUTGOING_DIRECT;
        indexOrDestination = commissioningGpd->gppShortAddress;
      } else {
        type = EMBER_OUTGOING_BROADCAST;
        indexOrDestination = EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS;
      }
    }
    retval = sendGpPairingMessage(type,
                                  indexOrDestination,
                                  pairingOptions,
                                  gpdAddr,
                                  groupId,
                                  ...
                                  sinkEntry.assignedAlias,
                                  sinkEntry.groupcastRadius);
    emberAfGreenPowerClusterPrintln("Gp Pairing %s %s send returned %d",
                                    (commissioningState.unicastCommunication ? "UCAST" : "BCAST"),
                                    (commissioningGpd->doNotSendGpPairing ? "to self" : ""),
                                    retval);
...
static void finalisePairing(GpCommDataSaved *commissioningGpd)
{
  ...
  if (handleGpdMatchingSinkFunctionality(commissioningGpd)) {
    ...
    if (commissioningGpd->commissionState == GP_SINK_COMM_STATE_SEND_COMM_REPLY) {
      ...
      sendCommissioningReply(&(commissioningGpd->addr),
                             commissioningGpd->securityLevel,
                             commissioningGpd->gpdfExtendedOptions,
                             commissioningGpd->gpdfOptions,
                             &linkKey,
      ...
      if (handleAddingTranstionTableEntry(commissioningGpd)) {
        handleSinkEntryAndPairing(&(commissioningGpd->addr),
                                  commissioningGpd->gpdfOptions,
                                  commissioningGpd->gpdfExtendedOptions,
                                  commissioningGpd->outgoingFrameCounter,
                                  &(commissioningGpd->applicationInfo),
                                  commissioningGpd);
  ...
  if (gpdCommandId == EMBER_ZCL_GP_GPDF_DECOMMISSIONING) {
    ...
    if (sinkFunctionalitySupported(EMBER_AF_GP_GPS_FUNCTIONALITY_TRANSLATION_TABLE)) {
      ...
      emGpForwardGpdCommandBasedOnTranslationTable(&gpdAddr,
                                                   ...
                                                   gpdCommandPayload);
      #else
      emGpForwardGpdToMappedEndpoint(&gpdAddr,
                                     gpdCommandId,
                                     gpdCommandPayload);
      #endif //EMBER_AF_PLUGIN_GREEN_POWER_TRANSLATION_TABLE
    }
                                                            ...
                                                            uint8_t *reportDescriptor)
{
  ...
  // Reset bound the following varibles to 0 if they are 0xFF, because in the autogenerator,
  // if the feild in the payload is not available it is set as 0xFF(invalid value).
  ...
  if (appInformationPresent) { // application info present
    ...
    if (applicationInformation & EMBER_AF_GP_APPLICATION_INFORMATION_GPD_APPLICATION_DESCRIPTION_COMMAND_FOLLOWS) {
      ...
      if (commissioningGpd->commissionState == GP_SINK_COMM_STATE_IDLE
  ...
  if ((groupListCount == 0xFF || groupList == NULL)
    ...
    if (actions & EMBER_AF_GP_PAIRING_CONFIGURATION_ACTIONS_SEND_GP_PAIRING) {
      ...
      if (gpPairingConfigCommunicationMode == EMBER_GP_SINK_TYPE_D_GROUPCAST) {
        ...
        dGroupId = emGpdAlias(&gpdAddr);
        EmberAfStatus UNUSED status = emGpAddToApsGroup(GP_ENDPOINT, dGroupId);
      }
      UNUSED EmberStatus retval;
      retval = sendGpPairingMessage(EMBER_OUTGOING_BROADCAST,
                                    EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS,
                                    pairingOptions,
                                    &gpdAddr,
                                    dGroupId,
